...
import { requireOwnedRoomOrNotFound } from "@/lib/authorization";
import { listRoomMessages, listRoomParticipants } from "@/lib/rooms";
...
const ROOM_TYPE_CONFIG: Record<string, { icon: string; color: string }> = {
  ...
  marketplace: { icon: "🏛️", color: "bg-emerald-500/15 text-emerald-300 border-emerald-500/25" },
  research: { icon: "🔬", color: "bg-sky-500/15 text-sky-300 border-sky-500/25" },
};

const DEMO_ROOMS: Record<
  string,
  {
    title: string;
    type: string;
...
> = {
  ...
  "demo-2": {
    title: "Design the perfect combat ruleset",
    type: "brainstorm",
    status: "active",
    bots: [
  ...
  if (id.startsWith("demo-")) {
    ...
    return (
      <RoomShell
        ...
        status={demo.status}
        isLive={isLive}
        turnCount={demo.messages.length}
        isDemo
      >
        <RoomInteractive
          roomId={id}
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-xs text-white/35">Round {turnCount}</span>
                <span className="text-xs text-white/25">·</span>
                <span className="text-xs text-white/35">Arena #{id.slice(0, 8)}</span>
              </div>
            </div>
          </div>
        </div>
